...
export const TOKEN_KEY = 'JSESSIONID'
...
let routerName = ''
let routerArray = []
export const getMenuByRouter1 = (list, access) => {
  routerName = localStorage.getItem('auth_info')
  initMenu(list)
  if (routerName === null || routerName === '' || routerName.length === 0) {
    return
  }
  routerArray = routerName.split(',')
  if (routerArray === null || routerArray === undefined || routerArray.length === 0) {
    return
  }
  let res = []
  forEach(list, item => {
    if (routerArray.indexOf(item.path) >= 0 && item.meta.type === 'menu') {
