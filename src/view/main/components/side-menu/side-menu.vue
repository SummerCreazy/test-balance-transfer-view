  ...
  <div class="side-menu-wrapper">
    ...
    <Menu ref="menu" v-show="!collapsed" :active-name="activeName" :open-names="openedNames" :accordion="accordion" :theme="theme" width="auto" @on-select="handleSelect">
      <template v-for="item in menuList">
        <template v-if="item.children && item.children.length === 1 && item.children[0].meta.type !== 'menu'">
        ...
        <template v-else>
          <side-menu-item v-if="item.children && item.children.length >= 1" :key="`menu-${item.name}`" :parent-item="item"></side-menu-item>
